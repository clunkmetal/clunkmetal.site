...
// при открытии сайта с готовой ссылкой (например clunk.ru/#poster) - сразу открыть нужную вкладку
const hashTab = window.location.hash.replace('#', '');
if (hashTab && document.getElementById(hashTab)) {
  openTab(hashTab);
  // браузер сам пытается проскроллить к элементу с этим id - откатываем скролл наверх
  window.scrollTo(0, 0);
}
